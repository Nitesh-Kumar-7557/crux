...
const ArgumentInput = ({argumentId}:{argumentId: number;}) => {
    ...
    async function handleAffirmativeBtn(){
      await api.post(`/comment/affirmative/${argumentId}`,{userId: user?.id, input});
      setInput('')
    }
    async function handleNegativeBtn(){
      await api.post(`/comment/negative/${argumentId}`,{userId: user?.id, input});
      setInput('')
    }
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-neutral-950/80 backdrop-blur-xl border-t border-outline-variant/20 py-6 px-6 z-40">
      <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <button onClick={handleAffirmativeBtn} className="flex-1 cursor-pointer md:flex-none border border-primary text-primary hover:bg-primary/10 px-8 py-4 font-label uppercase tracking-widest text-xs transition-all">
            Support Affirmative
          </button>
          <button onClick={handleNegativeBtn} className="flex-1 cursor-pointer md:flex-none border border-secondary text-secondary hover:bg-secondary/10 px-8 py-4 font-label uppercase tracking-widest text-xs transition-all">
            Support Negative
          </button>
        </div>
      </div>
