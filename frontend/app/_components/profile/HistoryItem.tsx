...
const HistoryItem = ({date,result,inFavour,title,score,replayLink}: DebateHistoryProps) => {
  return (
    <div className={`grid grid-cols-1 md:grid-cols-12 gap-4 p-6 bg-surface-container-low items-center border-l-4 ${result === 'win' ? 'border-primary':'border-secondary'}`}>
      <div className="md:col-span-2">
        <span className="font-label text-[10px] text-outline block mb-1">
          DATE / RESULT
        </span>
        <span className={`font-label text-sm ${result === 'win' ? 'text-primary':'text-secondary'} font-bold uppercase`}>
          {date} / {result}
        </span>
      </div>
